...
import datetime
import wandb
from util.util import train_log, refresh
...
if opt.isTrain and len(opt.gpu_ids) > 0:
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    if epoch % opt.save_epoch_freq == 0:
        if local_rank == 0:
            ...
            save_checkpoint(epoch, model.module, optimizer_warp, os.path.join(opt.checkpoints_dir, opt.name, 'PBAFN_warp_epoch_%03d.pth' % (epoch+1)))
